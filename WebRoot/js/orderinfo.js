...
function jiancha(index) {
	...
	case 1:
		...
		if ($("phone").value.trim() == "") {
			...
			$("dianhuaspan").style.display = "none";
		}

		// tianxieneirong = $("name").value.trim()+" , " +
		// $("dizhi").value.trim() + " , " + $("phone").value.trim()+ " , " +
		// $("youbian").value.trim();
		tianxieneirong = $("dizhi").value.trim() + " , "
				+ $("phone").value.trim() + " , " + $("beizhu").value.trim();
		break;
	case 2:
		var annius = document.getElementsByName("songhuofangshi");

		for (var ai = 0; ai < annius.length; ai++) {
			if (annius[ai].checked) {
				tianxieneirong = annius[ai].value + " , "
						+ $("songhuoshijian" + ai).value;
				var yunfei = 0;
				var zong = parseFloat($("zongerspan").innerHTML);
				if (ai == 0) {
					yunfei = zong > 29 ? 0 : 5;
				} else if (ai == 1) {
					yunfei = 10;
...
function checkvalidatecode() {
	...
	if (validatecode == "") {
		...
		// 配置XMLHttpRequest对象
		xmlHttp
				.open("get", "CheckValidateCode.aspx?validatecode="
						+ validatecode + "&flag=orderinfo&time="
						+ new Date().getTime());
